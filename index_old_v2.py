...
proxies = []
#proxies = rotate_proxy2(webdriver,len(backer_urls))
...
try:
    ...
    for url in backer_urls:
        ...
        #crawl backer's info
        def crawl_about_info(soup,backer_info):
            #biography
        ...
        #simulate manual human verification
        def click_and_hold():
            #verify security check
            #captcha_element = driver.find_element_by_id('px-captcha')
            captcha_element = driver.find_element_by_xpath('/html/body/section/div[3]/div/div[1]/div/iframe[1]')
            ActionChains(driver).click_and_hold(captcha_element).perform()
            #ActionChains(driver).pause(10)
            #ActionChains(driver).release().perform()

        try:
            crawl_about_info(soup,backer_info)
        except AttributeError:
            pass
        #Exception raise during display of human verification
        ...
        try:
            ...
            backed_link.click()
            #Check for 'Backed link' again to detect the need for human verification
            backed_link = driver.find_element_by_partial_link_text('Backed')
            time.sleep(7)
        #Exception raise during display of human verification
        except NoSuchElementException:
        ...
        #backer's info
        def crawl_backer_info(soup,backer_info):
            try:
        ...
        crawl_backer_info(soup,backer_info)

        #list of backer's projects
        project_list = []
        project_list = crawl_project_info(soup)
